...
conds = ['baseline', 'congruent', 'competition']
...
for cond in conds:
	...
	f = open("modeling results/EBM_aics_"+cond+"_adults.txt", "w+")
	#f = open("modeling results/EBM_aics_competition_adults.txt", "w+")
	#f = open("modeling results/EBM_aics_baseline_adults.txt", "w+")

	f.write('subj ebm_aic ebm_Bev ebm_weight Lag_aic Lag_Bev Lag_Blag\n')
	...
	#beh_data = openfile('competition_adults.txt', n_trials)
	beh_data = openfile(cond+'_adults.txt', n_trials)
	...
	for subj_data in beh_data[:]:
			...
			outputs =  [get_aic(ebm_nllh, 3), ebm_Bev, ebm_weight, get_aic(lag_nllh, 3), Bev, Blag]
